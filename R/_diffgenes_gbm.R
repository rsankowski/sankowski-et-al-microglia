...
mainDir <- getwd()
subDir <- file.path("data", "Cluster_specific_genes_gbm")
subsubDir_up <- 'Up'
...
up_genes <- load_data(file.path('data','Cluster_specific_genes_gbm',"Up"))
...
up_genes %>% 
  ...
  write.csv(file.path('data', 'Human_gbm_microglia_up_genes_padj<05_logfc>1.csv'))

up_genes2 %>% 
  filter(up_genes2$pval<0.05, up_genes2$log2FoldChange > 1) %>% 
  dplyr::arrange(Cluster) %>%
  dplyr::distinct(Cluster, GENEID) %>%
  write.csv(file.path('data','Human_gbm_micr_unique_up_genes_wo_stress-genes_log2fc>1.csv'))
