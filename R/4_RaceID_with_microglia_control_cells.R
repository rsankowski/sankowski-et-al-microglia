...
ids <- read_csv(file.path("data","microglia_nn.csv"))[[2]]
prdata <- prdata[, ids]
...
sc <- clustexp(sc,
               ...
               SE.factor=.25,
               B.gap=50,
               cln=9, #this cluster number was determined based on the graphical output of the plotsaturation function
               rseed=17000,
               FUNcluster="kmedoids",
               FSelect=TRUE)
# compute t-SNE map
...
save(sc, file = file.path("data", "sc_ctrl_microglia.RData"))

#please run this code if you would like to have the same t-SNE layout and clustering as the original paper
ctrl_micr_nn <- read.csv(file.path("data", "ctrl_microglia_clusters+embeddings_nn.csv"), row.names = 1)
sc@tsne <- ctrl_micr_nn[, c("V1", "V2") ]

clust_nn <- ctrl_micr_nn$Cluster
names(clust_nn) <- rownames(ctrl_micr_nn)

assertthat::assert_that(identical(names(sc@cpart), names(clust_nn)))
sc@cpart <- clust_nn
sc@tsne <- ctrl_micr_nn[,c("V1", "V2")]

save(sc, file = file.path("data", "sc_ctrl_microglia_nn.RData"))
