...
hyper_test <- function(data1 = sc, data2 = df, var1 = "Cluster", var2 = "Region") {
  require(tidyverse)            
  clusters <- data.frame(table(data1@cpart))
  colnames(clusters) <- c(var1, 'cluster_size')
  vars <- as.data.frame(table(data2[,var1], data2[,var2]))
...
#plots
plotexptsne2 <-   function(.gene = gene, .df=df, .sc=sc, point_size=3, logsc=FALSE, line_width=0, .retain_cl = retain_cl) {
  .gene = name2id(.gene, rownames(.sc@ndata))
  l <- colSums(.sc@ndata[rownames(.sc@ndata)[rownames(.sc@ndata) %in% .gene], colnames(.sc@ndata) %in% names(.sc@cpart)[.sc@cpart %in% .retain_cl]])
  mi <- min(l)
  ma <- max(l)
...
#make long dataset 
make_data_long <- function(data1 = df, data2 = sc@ndata, .genes=genes ,order_clusters = levels(df$Cluster)) {
  .genes <- name2id(.genes, rownames(data2))
  data_t <- data.frame(t(data2[.genes,]), 'ID' = colnames(data2))
  
  data_long <- data_t %>% gather('Gene', 'Expression', -ID) %>%
    left_join(df[,c(1,4)])
  
  #order clusters based on order_clusters
...
#line plot
gene_line_plot <- function(data = data_long, .gene = gene) {
...
plot_heatmap <- function(.sc=sc, .df=df, .gene_names=gene_names, .n=20, .colors = viridis(100), .retain_cl=retain_cl) {
  
  .gene_names <- unique(as.character(.gene_names$GENEID))
  
  nd <- .sc@ndata[.gene_names,]
  nd[is.na(nd)] <- 0
